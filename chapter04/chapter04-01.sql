...
-- 정수형은 number을 사용하여 문자형은 가변형 문자타입인 varchar을 사용한다
-- bookid(도서 번호) - number
-- bookname(도서 이름) - varchar2(20)
-- publisher(출판사) - varchar2(20)
-- price(가격) - number

-- 문자형 데이터 타입 - char, varchar, varchar2
-- char(n) - n바이트를 가진 문자형 타입이다
-- 저장되는 문자의 길이가 n보다 작으면 나머지는 공백으로 채워서
-- n바이트를 만들어 저장한다
-- 예) char(20)
-- varchar(n) - n바이트를 가진 문자열 타입이지만
-- 저장되는 문자의 길이 만큼만 기억장소를 차지하는 가변형이다
-- 예) varchar(20)
...
-- custid(고객번호) - number, not null 제약 조건, 
-- 외래키(newcustomer.custid, 인쇄삭제)
