...
-- 최근 버전인 dbms에서는 as 키워드를 생략할 수 있다
...
-- 투플의 개수 : 카디널리티
-- count(*)는 전체 투플의 수를 세어준다(특정 속성의 null값은 무시 한다)
-- count(publisher)는 출판사의 수를 세어주는데 null값은 제외된다.
-- count(distinct publisher)는 중복을 제거한 출판사의 수를 세어 준다(null값은 제외)
...
-- group by와 having 절 사용시 주의 사항
-- group by로 투플을 그룹으로 묶은 후 select절에는 
-- group by에서 사용한 속성과 집계함수인 나올 수 있다
select sum(saleprice), custid
from orders
group by custid;

-- where절과 having절이 같이 포함된 sql문은 검색조건이 모호해질 수 있따
-- having절 주의 사항
-- 1. 반드시 group by절과 같이 작성해야 한다
