-- sql 고급
-- 01. 내장함수
-- 내장 함수는 다음과 같이 select 절과 where절, update절 등에서
-- 모두 사용할 수 있다
...
-- 오라클에서 제공하는 내장 함수
-- 숫자 함수
-- sql문에서는 수학의 기본적인 사칙 연산자(+,-,*,/)외 나머지(%)연산자 기호를
-- 그대로 사용한다
-- 사칙 연산자 외에도 여러 연산자가 있다
-- 이러한 연산자 중 사용 빈도가 높은 것을 내장 함수 형태로 제공한다
...
-- round(숫자, m) 함수 - m자리를 기준으로 숫자 반올림
-- 4.875를 숫자 첫째 자리까지 반올림한 값을 구하시오
select round(4.875,1)
from dual;

-- 숫자 함수의 연산
-- 인자에 숫자 대신 열이름도 사용할 수 있고 여러 함수를 복합적으로
-- 사용할 수도 있다
-- 고객별 평균 주문 금액의 백 원 단 위로 반올림한 값을 구하시오
select custid "고객 번호", round(sum(saleprice) / count(*),-2) 평균금액 
...
-- length(s) 함수 - 대상 문자열의 글자 수를 반환
-- length - 문자의 수, lengthb - 바이트 단위로 변환
-- 굿스포츠에서 출판한 도서의 제목과 제목의 글자 수, 바이트 수를 보이시오
select bookname, length(bookname), lengthb(bookname)
from book
where publisher = '굿스포츠';

-- substr(s,n,k) 함수 - 대상 문자열의 오른쪽부터 지정된 자리에서부터
-- 지정된 길이만큼 잘라서 반환

-- 마당서점의 고객 중에서 같은 성을 가진 사람이 몇 명 이나 되는지
-- 성별 인원수를 구하시오
select substr(name,1,1), count(*)
from customer group by substr(name,1,1);
select * from customer;
insert into customer values(5,'박세라','대한민국 서울',null);
...
-- add_months(date,숫자);
-- 날짜에 지정한 달을 더해 date형으로 반환(1:다음달, -1:이번달)
-- last_day(date)
-- 날짜에 달의 마지막 날을 date형으로 반환
-- sysdate
-- dbms 시스템상의 당일 날짜를 date형으로 반환하는 인자가 없는 함수

-- 마당서점은 주문일로부터 10일 후 매출을 확장한다. 각 주문의 확장일자를 구하시오
...
-- dbms서버에 설정된 현재 날짜와 시간, 요일을 확인하시오
...
-- 3. 집계 함수에 적용되는 행이 하나도 없으면
-- sum, avg 함수의 결과는 null이 되고, count함수의 결과는 0이다
create table mybook(
...
-- nvl함수
-- nvl함수는 null 값을 다른 값으로 대처하여
-- 연산하거나 다른 값으로 출력
-- null값을 임의의 다른 값으로 변경할 수 있다
-- nvl(속성,값)형식으로 사용
-- 속성이 null 값이면 값으로 대처한다
...
-- 오라클의 select문 처리순서
-- 1. from customer - customer 테이블을 읽어들임
-- 2. where rownum <= 2 - 오라클이 데이터를 읽은 순서대로 두 개 두플 선택
