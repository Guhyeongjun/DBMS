-- 02. 부속질의
-- 일반적으로 데이터가 대항일 경우 데이터를 모두 합쳐서 연산하는 조인보다
...
-- 조인을 사용할 경우
-- customer테이블과 orders 테이블의 곡개번호로 조인한 후 필요한 데이터를 추출한다
-- 부속질의를 사용할 경우
-- customer 테이블에서 박지성 고객의 고객번호를 찾고
-- 찾은 고객번호를 바탕으로 orders 테이블에서 확인한다

-- 부속질의는 sql문 안에 다른 sql문이 중첩된 질의를 말한다
-- 주 질의(main query, 외부질의)와 부속질의(sub query, 내부질의)로 구성 된다
...
-- from절에서 결과를 뷰(view)형태로 반환하기 때문에 인아링 뷰라고 한다
...
-- 처리과정은 주 질의의 대칭 열 속성의 값과 부속질의의 결과값을
