...
-- 보조기억장치는 일반적으로 하드디스크,ssd,usb메모리들이 있다
...
-- 데이터를 나누어서 데이터 단위를 작게 만드는 것이 가장 좋은 튜닝이다
...
-- on 테이블이름(컬럼),(컬럼)
-- reverse는 역순으로 인덱스를 생성
-- unique는 테이블의 속성값에 대하여 중복이 없는 유일한 인덱스를 생성하는 것
-- book 테이블의 bookname 열의 대상으로 인덱스 ix_book를 생성하시오
